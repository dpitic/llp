/*
 * This program demonstrates tail recursion using a function that accesses
 * a linked list element by index. Compiling the code with -Os will produce
 * non-recursive code.
 */
#include <malloc.h>
#include <stdio.h>
...
struct llist *llist_at(struct llist *lst, size_t idx) {
  if (lst && idx) {
    return llist_at(lst->next, idx - 1);
  }
  return lst;
}
